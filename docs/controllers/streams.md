...
### Event Modification
You can modify raw objects in flight before they are passed on to a reflector or controller. This is usually done to minimise [reflector] memory consumption.
...
This is not important from a data-perspective (as you will see everything from `reconcile`), but it is the stream that back-propagates through the stream of streams that the `Controller` ultimately manages.
...
!!! warning "Metadata Watchers on Main Controller Stream"
